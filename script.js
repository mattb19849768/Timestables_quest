...
let questions=[], currentQ=0, score=0, streak=0, startTime, timerInterval;
...
function resizeCanvas(){bgCanvas.width=window.innerWidth; bgCanvas.height=window.innerHeight; confCanvas.width=window.innerWidth; confCanvas.height=window.innerHeight; fireCanvas.width=window.innerWidth; fireCanvas.height=window.innerHeight;}
...
avatarList.forEach(a=>{
    ...
    div.addEventListener('click',()=>{selectedAvatar=a; document.querySelectorAll('.avatar-choice').forEach(c=>c.classList.remove('selected')); div.classList.add('selected');});
...
function showQuestion(){
    ...
    if(multipleChoice){answersContainer.innerHTML=''; const correct=q.a*q.b;
        const options=[correct];
        while(options.length<4){const o=Math.floor(Math.random()*12*q.a)+1; if(!options.includes(o)) options.push(o);}
        shuffleArray(options);
        options.forEach(o=>{const btn=document.createElement('button'); btn.className='answer-btn'; btn.textContent=o;
